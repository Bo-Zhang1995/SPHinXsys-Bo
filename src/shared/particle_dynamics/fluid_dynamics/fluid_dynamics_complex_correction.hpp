...
void BaseIntegration2ndHalfCorrectWithWall<BaseIntegration2ndHalfCorrectType>::
    interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < FluidWallData::contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != wall_neighborhood.current_size_; ++n)
        {
            ...
            density_change_rate += (this->vel_[index_i] - vel_in_wall).dot(this->B_[index_i] * e_ij) * dW_ijV_j;
