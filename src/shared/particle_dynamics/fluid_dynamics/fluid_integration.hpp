...
BaseIntegration<DataDelegationType>::BaseIntegration(BaseRelationType &base_relation)
    : LocalDynamics(base_relation.getSPHBody()), DataDelegationType(base_relation),
      ...
      vel_div_(this->particles_->template registerStateVariable<Real>("VelocityDivergence")),
      ...
      correction_(particles_), riemann_solver_(fluid_, fluid_)
{
    ...
    particles_->addEvolvingVariable<Real>("VelocityDivergence");
...
void Integration1stHalf<Inner<>, RiemannSolverType, KernelCorrectionType>::interaction(size_t index_i, Real dt)
{
    ...
    vel_div_[index_i] = rho_dissipation;
}
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
    ...
    interaction(size_t index_i, Real dt)
{
    ...
    this->vel_div[index_i] += rho_dissipation;
}
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
Integration2ndHalf<Inner<>, RiemannSolverType, KernelCorrectionType>::
    Integration2ndHalf(BaseInnerRelation &inner_relation)
    : BaseIntegration<DataDelegateInner>(inner_relation), 
    correction_(particles_), riemann_solver_(this->fluid_, this->fluid_),
      mass_(particles_->getVariableDataByName<Real>("Mass")),
      Vol_(particles_->getVariableDataByName<Real>("VolumetricMeasure")) {}
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
void Integration2ndHalf<Inner<>, RiemannSolverType, KernelCorrectionType>::initialization(size_t index_i, Real dt)
{
    pos_[index_i] += vel_[index_i] * dt * 0.5;
}
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
void Integration2ndHalf<Inner<>, RiemannSolverType, KernelCorrectionType>::update(size_t index_i, Real dt)
{
    rho_[index_i] += drho_dt_[index_i] * dt * 0.5;
}
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
void Integration2ndHalf<Inner<>, RiemannSolverType, KernelCorrectionType>::interaction(size_t index_i, Real dt)
{
...
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
Integration2ndHalf<Contact<Wall>, RiemannSolverType, KernelCorrectionType>::
...
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
void Integration2ndHalf<Contact<Wall>, RiemannSolverType, KernelCorrectionType>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != wall_neighborhood.current_size_; ++n)
        {
            ...
            density_change_rate += (vel_[index_i] - vel_j_in_wall).dot(0.5 * (correction_(index_i) + correction_(index_j)) * e_ij) * dW_ijV_j;
...
//=================================================================================================//
template <class RiemannSolverType, class KerenlCorrectionType>
Integration2ndHalf<Contact<>, RiemannSolverType, KerenlCorrectionType>::
      ...
      correction_(this->particles_)
{
    for (size_t k = 0; k != contact_particles_.size(); ++k)
    {
        ...
        contact_corrections_.push_back(KerenlCorrectionType(contact_particles_[k]));
...
//=================================================================================================//
template <class RiemannSolverType, class KernelCorrectionType>
void Integration2ndHalf<Contact<>, RiemannSolverType, KernelCorrectionType>::interaction(size_t index_i, Real dt)
{
    Real density_change_rate = 0.0;
    Vecd p_dissipation = Vecd::Zero();
    for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
        {
            ...
            Real u_jump = (this->vel_[index_i] - vel_k[index_j]).dot(0.5 * (correction_(index_i) + correction_k(index_j)) * e_ij);
    ...
    this->vel_div_[index_i] += density_change_rate;
