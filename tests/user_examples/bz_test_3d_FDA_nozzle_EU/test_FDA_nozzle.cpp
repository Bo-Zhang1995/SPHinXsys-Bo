...
BoundingBox system_domain_bounds(
	Vec3d(-half_length - BW, -0.5 * nozzle_diameter - BW, -0.5 * nozzle_diameter),
	Vec3d(half_length + BW, 0.5 * nozzle_diameter + BW, 0.5 * nozzle_diameter + BW));
...
Real Q_f = 5.21e-6; // for Re = 500 = rho_f * u_th * throat_diameter / mu_f
Real U_f = Q_f * 4 / (PI * nozzle_diameter * nozzle_diameter);
Real U_max = 0.8; // From exp data
Real c_f = U_max * 10;
//----------------------------------------------------------------------
...
public:
	...
	void update(size_t index_i, Real dt)
	{
		...
		Real local_radius = pow(x, 2) + pow(y, 2);
		vel_[index_i][0] = 2 * U_f * (1 - 4 * local_radius / nozzle_diameter / nozzle_diameter);
	}
protected:
	StdLargeVec<Vecd>& vel_, & pos_;
};

// Observing points are generated here.
//----------------------------------------------------------------------
...
int main(int ac, char* av[])
{
	...
	if (sph_system.RunParticleRelaxation())
	{
		...
		BodyStatesRecordingToVtp write_water_body_to_plt(io_environment, { &water_body });
		BodyStatesRecordingToVtp write_boundary_body_to_plt(io_environment, { &outer_column });
	...
	Real D_Time = End_Time / 100;	 /**< Time stamps for output of body states. */
