...
BoundingBox system_domain_bounds(Vec3d::Zero(), Vec3d(DL, DH, DW));
//----------------------------------------------------------------------
...
public:
	explicit WaterBlock(const std::string& shape_name) : ComplexShape(shape_name)
	{
		...
		Vecd center_water(0.5 * DL, 0.5 * DH, 0.5 * DW);
...
public:
	...
	void update(size_t index_i, Real dt)
	{
		...
		rho_[index_i] = rho0_f;
		vel_[index_i][0] = cos(2.0 * Pi * pos_[index_i][0]) * sin(2.0 * Pi * pos_[index_i][1]) * sin(2.0 * Pi * pos_[index_i][2]);
		vel_[index_i][1] = -0.5 * sin(2.0 * Pi * pos_[index_i][0]) * cos(2.0 * Pi * pos_[index_i][1]) * sin(2.0 * Pi * pos_[index_i][2]);
		vel_[index_i][2] = -0.5 * sin(2.0 * Pi * pos_[index_i][0]) * sin(2.0 * Pi * pos_[index_i][1]) * cos(2.0 * Pi * pos_[index_i][2]);
...
int main(int ac, char* av[])
{
    ...
    if (sph_system.RunParticleRelaxation())
    {
       ...
       //----------------------------------------------------------------------
        ...
        PeriodicConditionUsingCellLinkedList periodic_condition_z(water_body, water_body.getBodyShapeBounds(), zAxis);
